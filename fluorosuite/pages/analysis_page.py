...
class _AnalysisTask(QRunnable):
    ...
    def run(self) -> None:
        if self.cancelled:
            ...
            return
        try:
            result_a = self._analyze_panel(self.inputs[0], self.parameters)
            result_b = (
                self._analyze_panel(self.inputs[1], self.parameters)
    ...
    def _analyze_panel(
        ...
        result = analyze_roi_residence_stream(frames, roi, parameters, fps)
